...
public class OrigamiIO {
    ...
    static public void write(Origami origami, String filename) throws Exception {
    ...
    static public Origami read2(String filename) throws Exception {
    ...
    /**
     *
     * @param filename	A fájl elérési útja az Origami Projects mappában
     * @deprecated {@link #read2(String)} will automatically try to read the
     * file in the old format.
     */
    static public Origami read1(String filename) throws Exception {
        ...
        try {
            ...
            File ori = new File(filename);
            FileInputStream str = new FileInputStream(ori);
            ...
            //ellenôrzi, hogy .ori fájl-e
            if (ored != 0x4f524544) {

                str.close();
                throw OrigamiException.H004;
            } else {

                int ver = str.read();

                //és hogy tudja-e olvasni ezt a verziót
                if (ver != 1) {
                    ...
                    while ((int) (car >> 32) != 0x46494c45) {

                        if (Double.longBitsToDouble(car) == 1.0) {

                            double[] cdr = new double[7];
                            cdr[0] = Double.longBitsToDouble(car);
                            for (int i = 1; i < 7; i++) {
                                ...
                                nextl *= 256;
                                nextl += str.read();
                                cdr[i] = Double.longBitsToDouble(nextl);
                            }
                            origami.history.add(cdr);
                        }
                        if (Double.longBitsToDouble(car) == 2.0) {

                            double[] cdr = new double[8];
                            cdr[0] = Double.longBitsToDouble(car);
                            for (int i = 1; i < 8; i++) {
                        ...
                        if (Double.longBitsToDouble(car) == 3.0) {
                        ...
                        if (Double.longBitsToDouble(car) == 4.0) {

                            double[] cdr = new double[9];
                            cdr[0] = Double.longBitsToDouble(car);
                            for (int i = 1; i < 9; i++) {
                        ...
                        if (Double.longBitsToDouble(car) == 5.0) {
